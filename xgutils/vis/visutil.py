...
def imageGrid(imgs, shape=None, zoomfac=1):
    for i in range(1, len(imgs)):
        if imgs[i].shape!=imgs[0].shape:
            ...
            imgs[i] = skimage.transform.resize(imgs[i], imgs[0].shape, anti_aliasing=True)
    imgs = np.array(imgs)
    if zoomfac!=1:
        if len(imgs.shape)==4:
...
def imgs2video2(imgs_dir, out_path, frameRate=6, ind_pattern="%d.png"):
    ''' Making video from a sequence of images
    ...
    command = command.format(outpath=out_path,imgs=os.path.join(imgs_dir,ind_pattern),framerate=frameRate)
    print('Executing command: ', command)
    _, stderr, returncode = sysutil.runCommand(command)
    if returncode!=0:
