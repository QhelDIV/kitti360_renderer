...
poses_data = np.loadtxt("%s/data_poses/%s/poses.txt" % (kitti360_root, sequence))
...
print(eye, lookat)
draw([*meshes])#, eye=meshes[0].get_center(), lookat=meshes[1].get_center(), up=[0,1,0])
# %%
import open3d as o3d
...
rgb_image, depth_image = o3d.geometry.create_rgbd_image_from_color_and_depth(mesh, intrinsic, extrinsic, depth_scale=1.0, depth_trunc=1000, convert_rgb_to_intensity=False)
